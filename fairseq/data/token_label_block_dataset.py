...
class TokenLabelBlockDataset(FairseqDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.include_targets:
            ...
            if s == 0:
                source = torch.cat([item.new([buffer[0].item()]), buffer[0 : e - 1]])
                past_target = torch.cat(
                    [item.new([buffer[0].item(), buffer[0].item()]), buffer[0 : e - 2]]
                )
            else:
                source = buffer[s - 1 : e - 1]
                if s == 1:
                    past_target = torch.cat([item.new([buffer[0].item()]), buffer[0 : e - 2]])
                else:
